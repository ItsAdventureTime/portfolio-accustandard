...
Deployment note: demo releases and `web-dist` stage under `/home/jk/bridge-ph/accustandard-demo`; Caddy serves them through a read-only bind mount at `/srv/bridge-ph-accustandard-demo` inside the container. A live deploy transactionally activates the `/demo/accustandard` redirect and installs the managed API handler from `deploy/caddy/Caddyfile.snippet` before the static fallback. The API uses `host.containers.internal:8080` because Caddy has no shared AccuStandard network; Caddy validation, reload, origin probes, and rollback are automated.
