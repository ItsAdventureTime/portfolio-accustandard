...
ssh -p "${REMOTE_PORT}" "${REMOTE_USER}@${REMOTE_HOST}" "grep -q 'srv/bridge-ph-accustanda-demo' ~/${REMOTE_CADDY_UNIT} || { echo 'Adding volume mount to caddy.container...'; sed -i '/# Bridge PH applications./a Volume=/home/jk/bridge-ph/accustanda-demo:/srv/bridge-ph-accustanda-demo:ro,Z' ~/${REMOTE_CADDY_UNIT} 2>/dev/null || sed -i '/Volume=.*delegateops-business/a Volume=/home/jk/bridge-ph/accustanda-demo:/srv/bridge-ph-accustanda-demo:ro,Z' ~/${REMOTE_CADDY_UNIT}; systemctl --user daemon-reload; }"
