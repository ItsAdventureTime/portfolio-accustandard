...
export default function DashboardHome() {
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 flex flex-col md:flex-row font-sans selection:bg-blue-600 selection:text-white">
      {/* Enterprise Sidebar Shell (Desktop & Mobile Drawer) */}
      <aside
        className={`${
          isSidebarOpen ? 'w-full md:w-64' : 'w-full md:w-20'
        } enterprise-sidebar transition-all duration-300 flex flex-col shrink-0 z-30`}
      >
        {/* Brand Lockup Header */}
        <div className="p-4 border-b border-slate-800 flex items-center justify-between">
          <div className={`flex items-center space-x-2 ${!isSidebarOpen && 'md:hidden'}`}>
            <div className="bg-white px-2.5 py-1 rounded-xl shadow-lg flex flex-col justify-center border border-slate-200">
              <div className="flex items-center leading-none">
                <span className="text-lg font-black text-blue-900 tracking-tighter">ACCUSTANDA</span>
                <span className="text-lg font-black text-red-600 ml-0.5">R<span className="text-xs font-black italic">x</span></span>
                <span className="text-lg font-black text-blue-900">D</span>
              </div>
              <div className="w-full border-b-2 border-red-600 my-0.5"></div>
              <span className="text-[6.5px] font-extrabold text-slate-900 tracking-tighter uppercase whitespace-nowrap">
                MEDICAL AND DIAGNOSTIC SUPPLIES CORP.
              </span>
            </div>
          </div>
          ...
          <button
            ...
            className="hidden md:flex p-2 text-slate-400 hover:text-white rounded-lg hover:bg-slate-800 transition"
        ...
        <nav className="flex-1 p-3 space-y-1.5 overflow-y-auto no-scrollbar">
          ...
          >
            ...
            <span className={!isSidebarOpen ? 'md:hidden' : ''}>Executive Overview</span>
          </button>
          ...
          >
            ...
            <span className={!isSidebarOpen ? 'md:hidden' : ''}>Inventory (QC & Pampanga)</span>
          </button>
          ...
          >
            ...
            <span className={!isSidebarOpen ? 'md:hidden' : ''}>Quotation Routing</span>
          </button>
          ...
          >
            ...
            <span className={!isSidebarOpen ? 'md:hidden' : ''}>Statement of Account</span>
          </button>
          ...
          >
            ...
            <span className={!isSidebarOpen ? 'md:hidden' : ''}>Purchasing & 3-Way Match</span>
          </button>
          ...
          >
            ...
            <span className={!isSidebarOpen ? 'md:hidden' : ''}>Request for Payment</span>
          </button>
          ...
          >
            ...
            <span className={!isSidebarOpen ? 'md:hidden' : ''}>Admin & Audit Logs</span>
          </button>
        </nav>

        {/* Sidebar Footer Indicator */}
        <div className={`p-4 border-t border-slate-800 ${!isSidebarOpen && 'md:hidden'}`}>
          <div className="flex items-center gap-2 text-[11px] text-emerald-400 bg-emerald-950/40 border border-emerald-800/50 p-2 rounded-xl">
            <Server className="w-3.5 h-3.5 text-emerald-400 shrink-0" />
            <span>QBO API Live Sync: OK</span>
          </div>
        </div>
      </aside>
      ...
      <div className="flex-1 flex flex-col min-w-0">
        {/* Top Header Bar */}
        <header className="sticky top-0 z-20 glass-header px-4 md:px-6 py-3 flex flex-wrap justify-between items-center gap-4">
          {/* Header Title */}
          <div>
            <h1 className="text-sm font-bold text-white uppercase tracking-wider flex items-center gap-2">
              <span>ACCUSTANDA ERP DASHBOARD</span>
              <span className="bg-blue-950 text-blue-300 border border-blue-800 text-[10px] px-1.5 py-0.5 rounded font-mono">v4.0</span>
            </h1>
            <p className="text-[11px] text-slate-400">COSO Control-First Multi-Location Medical Logistics</p>
          </div>
