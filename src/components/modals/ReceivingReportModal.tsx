...
export const ReceivingReportModal: React.FC<ReceivingReportModalProps> = ({ isOpen, onClose, poList, onReceivePO }) => {
  ...
  return (
    <AccessibleModal
      isOpen={isOpen}
      onClose={onClose}
      title="Enter receiving report"
      description="Verify goods receipt against the approved purchase order."
      size="lg"
    ...
    >
      <div className="modal-panel space-y-5 p-5 text-sm sm:p-8">
        ...
        {hardBlockError && <div id="receiving-report-error" role="alert" aria-live="assertive" aria-atomic="true" className="flex items-start gap-3 rounded-2xl border border-rose-300 bg-rose-50 p-4 text-xs font-bold text-rose-950"><AlertTriangle className="h-5 w-5 shrink-0 text-rose-600" /><span>{hardBlockError}</span></div>}
